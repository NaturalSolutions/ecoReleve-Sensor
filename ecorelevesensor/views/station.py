...
def insertMultStation(request):
    ...
    print('\n ___________ '+str(len(final)))
    # data_to_insert = pandas.DataFrame(data=final,)
    # data_to_insert = data_to_insert.sort(['date'])
    # last = data_to_insert['date'].iloc[-1].to_datetime()
    # first = data_to_insert['date'].iloc[0].to_datetime()
    # query = select([Station]).where(and_(Station.date <= last ,Station.date >= first ))

    # print (query)
    # res = DBSession.execute(query).fetchall()
    # print (res)
    # res = [OrderedDict(row) for row in res]
    # stations = pandas.DataFrame.from_records(res, index = ['date','LAT','LON'])
    # print (stations.iloc[0])

    # data = data_to_insert.append(stations)
    # data_duplicates = pandas.merge(data_to_insert,stations,how='inner', suffixes = ('__data','__sta') )

    # print (data_duplicates)

    # Create temporary Table and insert all waypoints
    tempName='tempTable'+str(request.authenticated_userid)+datetime.datetime.today().strftime('%Y%m%d%H%M%S')
    ...
    query='''DECLARE @data_duplicate table ( 
    data_id int,
    fk_sta_id int
    );

    INSERT INTO @data_duplicate 
    SELECT d.PK, s.TSta_PK_ID
    from {tableName} d join TStations s on d.LAT=s.LAT and d.LON = s.LON and d.date = s.DATE;

    DELETE {tableName}
    WHERE PK in (SELECT data_id FROM @data_duplicate);
    ...
    try :
        ...
        new = DBSession.query(tempTable.c['TSta_PK_ID']).all()
        nb = DBSession.execute(select([func.count()]).select_from(tempTable)).scalar()
        new = [row[0] for row in new]
        # -------------------------- #
        # Retrieve new station created 
        query=select([Station]).where(Station.id.in_(new))
