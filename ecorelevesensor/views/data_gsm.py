...
from sqlalchemy import desc, select, func,text, insert, join, Integer, cast, and_, Float, or_,bindparam, update
...
def data_gsm_unchecked_import(request):
	...
	query = update(DataGsm).where(DataGsm.id.in_(data)).values(validated = 1)
